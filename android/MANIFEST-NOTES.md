...
| `host` | `zeroclubs.xyz` | The verified origin. Must exactly match the host serving `/.well-known/assetlinks.json`. |
| `startUrl` | `/app` | Mirrors `start_url` in the web manifest, so the Android launcher and an installed PWA open the same screen. |
| `fullScopeUrl` | `https://zeroclubs.xyz/` | Mirrors web manifest `scope: "/"`. Anything inside this scope stays in the TWA; anything outside opens in a Custom Tab. Narrowing this would kick parts of the app out to the browser. |
...
| `additionalTrustedOrigins` | `www.zeroclubs.xyz` | `src/features/zeroForm/templates.ts` falls back to the `www` origin during SSR, so links can point there. Listing it keeps those navigations inside the app instead of bouncing to the browser. **If `www` is served, it must also host `assetlinks.json`.** |
