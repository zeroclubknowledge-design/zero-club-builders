...
--
-- This is the three migration files joined in order. Running it twice is safe:
-- every table is "create table if not exists", every function is "create or
-- replace", and every policy is dropped before it is created.
--
-- Until this has been run, ZeroStart shows "Could not find the table
-- public.zs_campaigns in the schema cache" — the app is deployed, but the
-- tables it reads do not exist yet.
-- ===========================================================================
...
-- Tell PostgREST the schema changed, so the app stops reporting missing tables.
notify pgrst, 'reload schema';
