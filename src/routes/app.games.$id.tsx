...
function ZeroGameCompetitionPage() {
  ...
  if (phase === "countdown") {
    return <CountdownScreen competition={competition} countdown={countdown} players={players} />;
  }
        ...
        <div>
          <section className="border-b border-border pb-6">
            ...
            <div className="mt-5 flex items-center gap-3">
              ...
              <div><p className="text-[11px] text-muted-foreground">Hosted by</p><p className="text-[13px] font-semibold">{profileName(competition.creator)}</p></div>
          ...
          <section className="mt-6">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              {players.map((player, index) => <PlayerLobbyCard key={player.id} player={player} hostId={competition.creator_id} index={index} />)}
...
function RaceScreen({ competition, players, currentProfileId, timeRemaining, submitting, onProgress, onSubmit, onLeave }: any) {
  return (
      ...
      <main className="mx-auto grid max-w-[1180px] gap-5 px-3 py-4 sm:px-5 md:py-6 lg:grid-cols-[minmax(0,1fr)_260px]">
        ...
        <aside className="order-first lg:order-none lg:sticky lg:top-20 lg:self-start">
          <div className="flex gap-2 overflow-x-auto pb-1 lg:block lg:space-y-2 lg:overflow-visible">
            {players.map((player: ZeroGamePlayer, index: number) => <PlayerProgress key={player.id} player={player} rank={index + 1} active={player.profile_id === currentProfileId} />)}
          </div>
        </aside>
      </main>
    </div>
...
function PlayerLobbyCard({ player, hostId, index }: { player: ZeroGamePlayer; hostId: string; index: number }) {
  const ready = player.status !== "joined";
  return <div className="flex h-[66px] items-center gap-3 rounded-md border border-border bg-card px-3"><Avatar profile={player.profile} size="sm" /><div className="min-w-0 flex-1"><div className="flex items-center gap-1.5"><p className="truncate text-[11.5px] font-semibold">{profileName(player.profile)}</p>{player.profile_id === hostId && <Crown className="h-3.5 w-3.5 fill-[#ffcf00] text-[#ffcf00]" />}</div><p className="mt-0.5 text-[9px] text-muted-foreground">Player {index + 1}</p></div><span className={`inline-flex items-center gap-1 text-[9px] font-semibold ${ready ? "text-emerald-600" : "text-muted-foreground"}`}>{ready && <Check className="h-3 w-3" strokeWidth={3} />}{ready ? "Ready" : "Waiting"}</span></div>;
}

function PlayerProgress({ player, rank, active }: { player: ZeroGamePlayer; rank: number; active: boolean }) {
  return <div className={`min-w-[190px] rounded-md border p-3 lg:min-w-0 ${active ? "border-primary bg-primary/[0.045]" : "border-border bg-card"}`}><div className="flex items-center gap-2.5"><span className="grid h-6 w-6 place-items-center rounded-sm bg-foreground/[0.06] text-[9px] font-semibold tabular-nums">{rank}</span><Avatar profile={player.profile} size="xs" /><p className="min-w-0 flex-1 truncate text-[10.5px] font-semibold">{active ? "You" : profileName(player.profile)}</p><span className="text-[9px] font-semibold tabular-nums text-muted-foreground">{player.progress}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-foreground/[0.06]"><div className="h-full rounded-full bg-primary transition-all duration-500" style={{ width: `${player.progress}%` }} /></div></div>;
}
