...
async function getMetricsData() {
  ...
  const [profileResult, postsResult, followersResult, followingResult, enrollmentResult, clubResult, questResult] = await Promise.all([
    supabase.from("profiles").select("id, username, full_name, xp, account_type, created_at").eq("id", userId).single(),
    supabase
      .from("posts")
  ...
  const insights = useMemo(() => {
    ...
    const activity = Array.from({ length: 7 }, (_, index) => {
      ...
      return {
        ...
        value: posts + builds + learning + connections,
      };
    });
    ...
    const nextAction = proofPosts.length === 0
    ...
    <div className="min-h-screen bg-background pb-28 md:pb-10">
        ...
        <div className="mx-auto flex h-[calc(4rem+env(safe-area-inset-top))] max-w-6xl items-end justify-between gap-3 px-4 pb-3 pt-[env(safe-area-inset-top)] md:px-7">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10.5px] text-muted-foreground">Your proof, progress, and momentum</p>
            </div>
          </div>
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-6 md:px-7 md:py-8">
        <section className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-primary">Builder record</p>
            <h2 className="mt-2 font-display text-[30px] font-semibold leading-[1.05] tracking-[-0.03em] text-foreground md:text-[38px]">
              Keep the signal moving, {profileName.split(" ")[0]}.
            </h2>
          </div>
        ...
        <section className="mt-5 grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
          <article className="rounded-2xl bg-card p-5 shadow-soft ring-1 ring-border md:p-6">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h3 className="mt-1.5 text-[18px] font-semibold tracking-tight text-foreground">Your last 7 days</h3>
              </div>
            ...
            <div className="mt-6 flex flex-wrap gap-x-4 gap-y-2 text-[11px] text-muted-foreground">
              <span className="flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-primary" />Posts and builds</span>
              <span className="flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-primary/55" />Learning and connections</span>
            </div>
          </article>
          ...
          <article className="overflow-hidden rounded-2xl bg-[#171417] p-5 text-white shadow-lift md:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.13em] text-white/50">Proof quality</p>
                <h3 className="mt-1 text-[18px] font-semibold tracking-tight">Your builder record</h3>
              </div>
            </div>
            <div className="mt-7 grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-white/[0.07] p-3 ring-1 ring-white/10">
                <p className="text-[23px] font-semibold tabular-nums">{insights.verifiedBuilds.length}</p>
                <p className="mt-1 text-[10px] text-white/55">Verified builds</p>
              </div>
              <div className="rounded-xl bg-white/[0.07] p-3 ring-1 ring-white/10">
                <p className="text-[23px] font-semibold tabular-nums">{data?.quests.length || 0}</p>
                <p className="mt-1 text-[10px] text-white/55">Quests claimed</p>
              </div>
              <div className="rounded-xl bg-white/[0.07] p-3 ring-1 ring-white/10">
                <p className="text-[23px] font-semibold tabular-nums">{data?.enrollments.length || 0}</p>
                <p className="mt-1 text-[10px] text-white/55">Bootcamps joined</p>
              </div>
              <div className="rounded-xl bg-white/[0.07] p-3 ring-1 ring-white/10">
                <p className="text-[23px] font-semibold tabular-nums">{data?.clubs.length || 0}</p>
                <p className="mt-1 text-[10px] text-white/55">Active clubs</p>
              </div>
            </div>
          </article>
        </section>
        ...
        <section className="mt-5 grid gap-4 lg:grid-cols-2">
          <article className="rounded-2xl bg-card p-5 shadow-soft ring-1 ring-border md:p-6">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.13em] text-muted-foreground">Top proof</p>
                <h3 className="mt-1.5 text-[18px] font-semibold tracking-tight text-foreground">Your strongest published work</h3>
              </div>
            ...
            {insights.topPost ? (
              ...
              <div className="mt-6 rounded-xl bg-foreground/[0.03] p-4 text-[13px] leading-6 text-muted-foreground ring-1 ring-border">Your published work will appear here once it starts gathering signal.</div>
            )}
          </article>
