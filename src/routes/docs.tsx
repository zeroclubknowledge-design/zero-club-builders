...
function DocsPage() {
  ...
  const filteredPages = useMemo(() => {
    ...
    return docPages.filter((item) =>
      [item.title, item.summary, item.group, ...item.sections.map((section) => section.title)]
        .join(" ")
        .toLowerCase()
        .includes(term),
    );
      ...
      <div className="p-4">
        <label className="flex h-10 items-center gap-2 rounded-md border border-[#171717]/10 dark:border-white/10 bg-white dark:bg-[#141118] px-3 focus-within:border-[#cc208f]/40 focus-within:ring-2 focus-within:ring-[#cc208f]/10">
          ...
          <input
            ...
            className="min-w-0 flex-1 bg-transparent text-[11.5px] outline-none placeholder:text-[#9b9599]"
          />
          {query && (
            <button type="button" onClick={() => setQuery("")} aria-label="Clear search">
              <X className="h-3.5 w-3.5 text-[#827b80] dark:text-white/55" />
            </button>
          )}
        </label>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-5" aria-label="Documentation pages">
        {Object.entries(groups).map(([group, pages]) => (
          <section key={group} className="mb-6">
            <p className="mb-2 px-2 text-[8.5px] font-semibold uppercase tracking-[0.15em] text-[#918a8e] dark:text-white/50">
              {group}
            </p>
        ...
        {filteredPages.length === 0 && (
          <div className="px-2 py-8 text-center">
            ...
            <p className="mt-1 text-[10px] leading-4 text-[#827b80] dark:text-white/55">
              Try a product name or action.
            </p>
          </div>
        )}
      </nav>
  ...
  return (
    <div className="min-h-screen bg-[#f7f6f3] dark:bg-[#100e13] font-sans text-[#171717] dark:text-white">
      ...
      <div className="sticky top-16 z-30 flex h-12 items-center justify-between bg-[#f7f6f3]/95 dark:bg-[#100e13]/95 px-4 backdrop-blur-xl lg:hidden">
        <button
          type="button"
          onClick={() => setMobileNavOpen(true)}
          className="flex items-center gap-2 text-[11px] font-semibold text-[#171717] dark:text-white"
        >
          <Menu className="h-4 w-4" /> All docs
        </button>
        <span className="max-w-[58vw] truncate text-[10.5px] font-medium text-[#6f696d] dark:text-white/55">
          {page.title}
        </span>
      </div>
      ...
      {mobileNavOpen && (
        <div
          className="fixed inset-0 z-[70] bg-black/25 lg:hidden"
      ...
      <div className="mx-auto max-w-[1440px] lg:grid lg:grid-cols-[260px_minmax(0,1fr)_210px]">
        <aside className="sticky top-16 hidden h-[calc(100vh-4rem)] border-r border-[#171717]/[0.08] dark:border-white/10 lg:block">
          {navigation}
        </aside>
        ...
        <main className="min-w-0 bg-white dark:bg-[#141118]">
          <article className="mx-auto max-w-[760px] px-5 py-12 sm:px-8 md:py-16 lg:px-12 lg:py-20">
            <div className="flex items-center gap-2 text-[9px] font-semibold uppercase tracking-[0.16em] text-[#9d176d]">
              ...
              <span className="flex items-center gap-1 text-[#827b80] dark:text-white/55">
                <Clock3 className="h-3 w-3" /> {page.readTime}
              </span>
            </div>
            <h1 className="mt-5 font-display text-[38px] font-semibold leading-[1.06] tracking-[-0.035em] sm:text-[48px]">
              {page.title}
            </h1>
            <p className="mt-5 max-w-[680px] text-[15px] leading-7 text-[#686266] sm:text-[16px]">
              {page.summary}
            </p>
            ...
            <div className="mt-10 space-y-12">
              {page.sections.map((section) => (
                <section key={section.title} id={sectionId(section.title)} className="scroll-mt-28">
                  <h2 className="font-display text-[24px] font-semibold tracking-[-0.02em] sm:text-[27px]">
                    {section.title}
                  </h2>
                  {section.paragraphs?.map((paragraph) => (
                    <p
                      key={paragraph}
                      className="mt-4 text-[13.5px] leading-7 text-[#565156] dark:text-white/60 sm:text-[14px]"
                    >
                      {paragraph}
                    </p>
                  ))}
                  {section.bullets && (
                    <ul className="mt-5 space-y-3">
                      {section.bullets.map((bullet) => (
                        <li
                          key={bullet}
                          className="flex items-start gap-3 text-[13px] leading-6 text-[#565156] dark:text-white/60 sm:text-[13.5px]"
                        >
                  ...
                  {section.note && (
                    <div className="mt-6 border-l-2 border-[#cc208f] bg-[#cc208f]/[0.045] px-4 py-3.5">
                      <p className="text-[12.5px] font-medium leading-6 text-[#4f484d]">
                        {section.note}
                      </p>
                    </div>
                  )}
            ...
            <div className="mt-16 grid gap-px overflow-hidden rounded-md border border-[#171717]/10 dark:border-white/10 bg-[#171717]/10 sm:grid-cols-2">
              {previousPage ? (
                <Link
                  to="/docs"
                  search={{ page: previousPage.slug }}
                  className="group bg-white dark:bg-[#141118] p-5 hover:bg-[#faf9f7]"
                >
                  <span className="flex items-center gap-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#8a8388] dark:text-white/45">
                    <ArrowLeft className="h-3 w-3" /> Previous
                  </span>
                  <span className="mt-2 block text-[12px] font-semibold group-hover:text-[#9d176d]">
                    {previousPage.title}
                  </span>
                </Link>
              ) : (
              ...
              {nextPage && (
                <Link
                  to="/docs"
                  search={{ page: nextPage.slug }}
                  className="group bg-white dark:bg-[#141118] p-5 text-left sm:text-right"
                >
                  <span className="flex items-center gap-1 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#8a8388] dark:text-white/45 sm:justify-end">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-between gap-3/10 dark:border-white/10 pt-6">
              ...
              <Link
                ...
                className="inline-flex items-center gap-2 text-[10.5px] font-semibold text-[#9d176d]"
              >
                Open Zero Club <ArrowRight className="h-3.5 w-3.5" />
              </Link>
            </div>
          </article>
        </main>
        ...
        <aside className="sticky top-16 hidden h-[calc(100vh-4rem)] border-l border-[#171717]/[0.08] dark:border-white/10 px-6 py-10 xl:block">
          <p className="text-[8.5px] font-semibold uppercase tracking-[0.15em] text-[#918a8e] dark:text-white/50">
            On this page
          </p>
          <nav className="mt-4 space-y-3" aria-label="On this page">
            {page.sections.map((section) => (
              <a
                ...
                className="block text-[10.5px] leading-4 text-[#777075] hover:text-[#9d176d]"
              >
                {section.title}
              </a>
            ))}
          ...
          <div className="mt-8/10 dark:border-white/10 pt-6">
            ...
            <Link
              to="/explore/$slug"
              params={{ slug: "feed" }}
              className="mt-3 inline-flex items-center gap-1 text-[10px] font-semibold text-[#9d176d]"
            >
