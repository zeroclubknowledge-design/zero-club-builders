...
function ClaimGiftPage() {
  ...
  return (
    <div className="min-h-screen bg-background px-4 py-[calc(2rem+env(safe-area-inset-top))] md:px-7">
      <main className="mx-auto grid max-w-[920px] gap-7 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-center">
        <section><p className="text-[10px] font-semibold uppercase text-primary">A Zero Club Gift for you</p><h1 className="mt-3 font-display text-[31px] font-semibold leading-tight tracking-tight sm:text-[40px]">Someone is backing your next step.</h1><p className="mt-3 max-w-xl text-[14px] leading-relaxed text-muted-foreground">{data.creator?.full_name || data.creator?.username || "A Zero Club member"} sent you restricted credit for {service?.label || card.service}. Claim it once and use it only for that purpose.</p><div className="mt-6 flex items-start gap-3 rounded-lg border border-primary/20 bg-primary/[0.045] p-4"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><p className="text-[12px] leading-relaxed text-muted-foreground">This claim is tied to your current Zero Club account and cannot be transferred after claiming.</p></div></section>
        <aside><GiftCardVisual amount={card.amount} service={card.service} templateId={card.template_id} code={card.code} message={card.message} /><button onClick={() => claimGift.mutate()} disabled={claimGift.isPending} className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-primary text-[14px] font-semibold text-primary-foreground disabled:opacity-50">{claimGift.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <>Claim this gift<ArrowRight className="h-4 w-4" /></>}</button></aside>
      </main>
    </div>
  );
}
