/**
 * The bootcamp form, lifted out of the route that used to own it.
 *
 * It lived in routes/app.tutor-studio.create.tsx and was imported from there
 * by routes/app.bootcamps_.$id.edit.tsx. One route file importing a value out
 * of another defeats the router's code splitter: the generated route tree
 * imports every route file statically, so a route file that exports anything
 * besides `Route` is pulled into the shared entry chunk along with everything
 * it touches. This form is around 50 kB, and it was being downloaded by every
 * visitor before the first screen could render.
 *
 * Living here, it is reached only through the two routes that use it.
 */
...
export function BootcampForm({
  ...
  const [modules, setModules] = useState([
    {
      id: "m1",
      title: "Introduction",
      lessons: [{ id: "l1", title: "Welcome and Orientation", type: "text" }],
    },
  ]);
      ...
      setPrimaryCouponId(
        (fetchedCoupons || []).find(
          (coupon: any) =>
            ...
            String(bootcamp.coupon_code || "").toUpperCase(),
        )?.id,
      );
      setExtraCoupons(
        (fetchedCoupons || [])
          ...
          .filter(
            (c: any) => (c.code || "").toUpperCase() !== (bootcamp.coupon_code || "").toUpperCase(),
          )
          .map((c: any) => ({
            key: c.id,
            id: c.id,
  ...
  const couponPreviewPrice = Math.max(
    0,
    Math.round(numericPrice * (1 - numericCouponDiscount / 100)),
  );
    ...
    try {
      ...
      const bootcampPayload = {
        ...
        coupon_discount_percent:
          couponEnabled && !isFree && normalizedCouponCode ? numericCouponDiscount : 0,
      ...
      if (savedBootcampId) {
        try {
          const couponIdsToRemove = Array.from(
            new Set([
              ...removedCouponIds,
              ...((!couponEnabled || isFree || !normalizedCouponCode) && primaryCouponId
                ? [primaryCouponId]
                : []),
            ]),
          );
            ...
            ...extraCoupons
              ...
              .map((c) => ({
                ...
                expires_at: c.expires_at
                  ? new Date(`${c.expires_at}T23:59:59`).toISOString()
                  : null,
                created_by: user.id,
              })),
          ];
      ...
      toast.success(
        bootcampId ? "Bootcamp updated successfully" : "Bootcamp launched successfully!",
      );
      ...
      if (!bootcampId) {
        toast("Collect learners before you start", {
          ...
          action: {
            ...
            onClick: () =>
              navigate({ to: "/app/tutor-studio", search: { view: "zero-forms" } as any }),
          },
          duration: 9000,
        });
      }
      ...
      modules.map((m) =>
        m.id === moduleId
          ? {
              ...
              lessons: [
                ...m.lessons,
                { id: Math.random().toString(), title: "New Topic", type: "text" },
              ],
            }
          : m,
      ),
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="text-[18px] font-semibold tracking-tight text-foreground">
                {bootcampId ? "Edit bootcamp" : "Create bootcamp"}
              </h1>
            </div>
          </div>
          <button
            onClick={saveDraft}
            className="flex h-10 items-center gap-1.5 rounded-lg border border-border bg-card px-3 text-[12px] font-semibold text-foreground tap hover:bg-muted sm:px-4"
          >
        ...
        {step === 1 && (
          <div className="animate-in space-y-7 fade-in slide-in-from-bottom-4 duration-500">
            <div className="space-y-2">
              <p className="text-[10px] font-semibold uppercase text-primary">Step 1 of 3</p>
              <h2 className="text-[23px] font-semibold tracking-tight">Bootcamp basics</h2>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="ml-1 text-[10.5px] leading-4 text-muted-foreground">
                  Use the toolbar to add headings, bold text and bullet points so learners can scan
                  it quickly.
                </p>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      >
                        {!isFree && <div className="h-2 w-2 rounded-full bg-primary" />}
                      </div>
                    ...
                    <div className="space-y-2 border-t border-border pt-4">
                      ...
                      {extraCoupons.map((coupon, index) => {
                        const patch = (changes: Partial<typeof coupon>) =>
                          setExtraCoupons((current) =>
                            current.map((c, i) => (i === index ? { ...c, ...changes } : c)),
                          );
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              <div className="relative w-[86px] shrink-0">
                                ...
                                />
                                <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[11px] font-bold text-muted-foreground">
                                  %
                                </span>
                              </div>
                              <button
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground mt-0.5">Publicly listed</p>
                    </div>
                  </div>
